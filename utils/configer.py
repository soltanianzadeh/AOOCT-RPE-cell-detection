...
class Configer(object):
    ...
    ### added by somayyeh soltanian-zadeh ###
    def save_config(self, ck_name):
        saveas =self.params_root.get('network')['model_dir']
        if not os.path.exists(saveas):
            os.makedirs(saveas)
            
        j = json.dumps(self.params_root, indent = 4)
        
        with open(os.path.join(saveas,"Config_"+ck_name+".json"), "w") as outfile:
